...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (method === 'POST' && action === 'login') {
    ...
    return json({ accessToken, refreshToken, expiresIn: '15m', user: { id: user.id, fullName: user.fullName, username: user.username, role: roleName } }, 200, origin);
  }
  ...
  if (method === 'POST' && action === 'refresh') {
    ...
    try { payload = jwt.default.verify(refreshToken, SECRET) as Record<string, unknown>; } catch { return error('Invalid refresh token', 401, origin); }
  ...
  if (method === 'POST' && action === 'logout') {
    ...
    return json({ message: 'Logged out successfully' }, 200, origin);
  }
  ...
  if (method === 'GET' && action === 'me') {
    ...
    return json({ id: user.id, fullName: user.fullName, username: user.username, role: roleName, branchId: user.branchId, hasPinCode: !!user.pinCode }, 200, origin);
  }

  if (method === 'POST' && action === 'verify-pin') {
    const { userId, pin } = req.body;
    const { data: user } = await supabase.from('users').select('*').eq('id', userId).single();
    if (!user) return error('Пользователь не найден', 404, origin);
    if (!user.pinCode) return error('PIN не установлен', 400, origin);
    if (user.pinCode !== pin) return error('Неверный PIN', 401, origin);
    return json({ message: 'PIN верный', valid: true }, 200, origin);
  }
  ...
  if (method === 'GET' && action === 'users-for-pin') {
    ...
    return json((users ?? []).map((u: Record<string, unknown>) => ({
