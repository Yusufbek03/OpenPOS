...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!path) return json(res, { message: 'Kitchen API v1.0' }, 200, origin);

  if (entityPath === 'stations') return handleStations(req, res, origin);
  if (entityPath === 'tickets') return handleTickets(req, res, origin);
